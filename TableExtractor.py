...
class TableExtractor:
    ...
    def execute(self):
        ...
        self.store_process_image("result/result.jpg", self.perspective_corrected_image_with_padding)
        return self.perspective_corrected_image

    def read_crop_image(self):
        self.image = cv2.imread(self.image_path)
        #self.image = self.image[self.image.shape[0]//2:self.image.shape[0], 0:self.image.shape[1]]
    ...
    def find_contours(self):
        self.contours, self.hierarchy = cv2.findContours(self.dilated_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    def apply_perspective_transform(self):
        ...
        self.perspective_corrected_image = cv2.warpPerspective(self.thresholded_image, matrix, (self.new_image_width, self.new_image_height))

    def add_10_percent_padding(self):
        image_height = self.image.shape[0]
        self.perspective_corrected_image = cv2.rectangle(self.perspective_corrected_image, (0,0), (self.perspective_corrected_image.shape[1], self.perspective_corrected_image.shape[0]), 0, 5)
        padding = int(image_height * 0.10)
        #self.perspective_corrected_image = cv2.convertScaleAbs(self.perspective_corrected_image, alpha=30, beta=1)
